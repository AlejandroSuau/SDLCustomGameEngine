...
void Particle::ReplaceParticle(Particle& new_particle) {
    type_ = new_particle.GetType();
    color_ = new_particle.GetColor();
    grid_ticks_count_ = new_particle.GetGridTicksCount();
}

void Particle::IncreaseGridTicksCount() {
    ++grid_ticks_count_;
}
...
std::size_t Particle::GetGridTicksCount() const {
    return grid_ticks_count_;
}
